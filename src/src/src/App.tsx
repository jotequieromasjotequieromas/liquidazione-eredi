...
export default function App() {
  ...
  // Estraggo immagini se è immagine, altrimenti provo a mostrare direttamente (per i PDF dipende dal contenuto)
  async function preparaAnteprima() {
    ...
    try {
      if (isImg) {
        ...
        // PDF: mostriamo un messaggio e chiediamo un’immagine di test
        setImgUrls([]);
        setStatus("Anteprima PDF non disponibile qui. Carica un'immagine (JPG/PNG) della pagina manoscritta per provare l’OCR.");
      }
    } catch {
      setStatus("Errore conversione");
    }
  }

  async function leggiQuestaPagina() {
    if (!imgUrls.length) {
      alert("Nessuna pagina/immagine da leggere");
      return;
    }
